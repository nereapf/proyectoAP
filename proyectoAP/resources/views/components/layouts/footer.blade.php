    ...
    <div class="px-4 py-4 flex flex-col md:flex-row items-center">
        <div class="flex flex-col items-center md:items-start md:w-1/3">
            <div class="flex items-center space-x-2">
                <img src="{{ asset('images/logoCompleto.png') }}" alt="Accesorios Pellés" class="w-70 h-40">
            </div>
        </div>
        ...
        <div class="md:w-1/3 flex flex-col items-center md:items-left space-y-3">
            ...
            @auth
                <form action="{{route("logout")}}" method="POST">
                    @csrf
                    <input type="submit" value="Cerrar sesión" class="mt-2 inline-block bg-azulBoton text-white text-xs px-2 py-1 rounded hover:bg-azulOscuro transition opacity-60">
                </form>
            @else
                <a href="{{route("login")}}"
