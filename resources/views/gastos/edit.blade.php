...
<div class="bg-azulFondo min-h-screen flex items-center justify-center pt-20 pb-10">
    <div class="relative w-full max-w-sm">
        ...
        <div class="bg-white rounded-xl pt-20 pb-8 px-8 shadow-md">
            <h2 class="text-2xl font-bold text-center mb-6">Editar Gasto</h2>
            <form onsubmit=event.preventDefault() action="{{ route('gastos.update', $gasto->id) }}" id="formulario{{$gasto->id}}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <x-text-input id="nombre" class="mt-1 block w-full rounded border-gray-300 focus:border-blue-700 focus:ring-blue-700"
                                  type="text" name="nombre" value="{{$gasto->nombre}}"/>
                    @error("nombre")
                ...
                <div>
                    ...
                    <x-text-input id="precio_hora" class="mt-1 block w-full rounded border-gray-300 focus:border-blue-700 focus:ring-blue-700"
                                  type="number" name="precio_hora" min="0" step="0.01" value="{{$gasto->precio_hora}}"/>
                    @error("precio_hora")
                    <div class="text-sm text-red-600">{{$message}}</div>
                    @enderror
                </div>
                ...
                <div class="flex space-x-2">
                    <button type="button" onclick="confirmUpdate({{$gasto->id}})"
...
<script>
    function confirmUpdate(id) {
        swal({
            title: "¿Deseas actualizar este gasto?",
